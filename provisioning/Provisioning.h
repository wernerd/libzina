...
namespace zina {
    ...
    public:
         ...
         */
        static int32_t registerZinaDevice(const std::string &request, const std::string &authorization,
                                          const std::string &scClientDevId, std::string *result);
         ...
         *
         * If the number of available pre-keys get to low then a client should generate and publish
         * new pre-keys.The server appends the pre-keys to the remaining existing pre-keys.
         *
         * @param store The persistent ZINA store to store and retrieve state information.
         * @param longDevId the unique device id of one of the user's registered ZINA devices
         * @param authorization authorization data, required to identify the user/device for which to append
         *        the new pre-keys.
