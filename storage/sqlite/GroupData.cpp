...
int32_t SQLiteStoreConv::createGroupTables()
{
    ...
    // Create Group table
    SQLITE_PREPARE(db, dropGroups, -1, &stmt, nullptr);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    SQLITE_CHK(SQLITE_PREPARE(db, createGroups, -1, &stmt, nullptr));
    ...
    // Create member table
    SQLITE_PREPARE(db, dropMembers, -1, &stmt, nullptr);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    SQLITE_CHK(SQLITE_PREPARE(db, createMembers, -1, &stmt, nullptr));
    sqlResult = sqlite3_step(stmt);
    if (sqlResult != SQLITE_DONE) {
        ERRMSG;
        goto cleanup;
    ...
    SQLITE_PREPARE(db, dropChangeSets, -1, &stmt, nullptr);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);

    SQLITE_CHK(SQLITE_PREPARE(db, createChangeSets, -1, &stmt, nullptr));
    sqlResult = sqlite3_step(stmt);
    if (sqlResult != SQLITE_DONE) {
        ERRMSG;
        goto cleanup;
...
int32_t SQLiteStoreConv::updateGroupDataDb(int32_t oldVersion)
{
    ...
    if (oldVersion == 4) {
        SQLITE_PREPARE(db, createGroups, -1, &stmt, nullptr);
        ...
        SQLITE_PREPARE(db, createMembers, -1, &stmt, nullptr);
        sqlCode_ = sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        if (sqlCode_ != SQLITE_DONE) {
    ...
    if (oldVersion == 6) {
        SQLITE_PREPARE(db, "ALTER TABLE groups ADD COLUMN burnTime INTEGER;", -1, &stmt, nullptr);
        sqlCode_ = sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        if (sqlCode_ != SQLITE_DONE) {
        ...
        SQLITE_PREPARE(db, "ALTER TABLE groups ADD COLUMN burnMode INTEGER;", -1, &stmt, nullptr);
        sqlCode_ = sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        if (sqlCode_ != SQLITE_DONE) {
        ...
        SQLITE_PREPARE(db, "ALTER TABLE groups ADD COLUMN avatarInfo VARCHAR;", -1, &stmt, nullptr);
        sqlCode_ = sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        if (sqlCode_ != SQLITE_DONE) {
    ...
    if (oldVersion == 7) {
        SQLITE_PREPARE(db, createChangeSets, -1, &stmt, nullptr);
        sqlCode_ = sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        if (sqlCode_ != SQLITE_DONE) {
...
int32_t SQLiteStoreConv::insertGroup(const string &groupUuid, const string &name, const string &ownerUuid, string& description, int32_t maxMembers)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, insertGroupsSql, -1, &stmt, nullptr));
...
int32_t SQLiteStoreConv::deleteGroup(const string &groupUuid)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, removeGroup, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
bool SQLiteStoreConv::hasGroup(const string &groupUuid, int32_t *sqlCode) {
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, hasGroupSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
static JSONUnique createGroupJson(sqlite3_stmt *stmt)
{
    auto jsn = make_unique<json>(json());

    // name is usually the SC UID string
    jsn->emplace(GROUP_ID, (const char*)sqlite3_column_text(stmt, 0));
    jsn->emplace(GROUP_NAME, (const char*)sqlite3_column_text(stmt, 1));
    jsn->emplace(GROUP_OWNER, (const char*)sqlite3_column_text(stmt, 2));
    jsn->emplace(GROUP_DESC, (const char*)sqlite3_column_text(stmt, 3));
    jsn->emplace(GROUP_MAX_MEMBERS, sqlite3_column_int(stmt, 4));
    jsn->emplace(GROUP_MEMBER_COUNT, sqlite3_column_int(stmt, 5));
    jsn->emplace(GROUP_ATTRIBUTE, sqlite3_column_int(stmt, 6));
    jsn->emplace(GROUP_MOD_TIME, sqlite3_column_int64(stmt, 7));
    jsn->emplace(GROUP_BURN_SEC, sqlite3_column_int64(stmt, 8));
    jsn->emplace(GROUP_BURN_MODE, sqlite3_column_int(stmt, 9));
    auto c_str =  (const char*)sqlite3_column_text(stmt, 10);
    jsn->emplace(GROUP_AVATAR, c_str != nullptr ? c_str : "");
...
int32_t SQLiteStoreConv::listAllGroups(list<JSONUnique> &groups)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectAllGroups, -1, &stmt, nullptr));
    ...
    while (sqlResult == SQLITE_ROW) {
        ...
        groups.push_back(createGroupJson(stmt));
        sqlResult = sqlite3_step(stmt);
    }
...
int32_t SQLiteStoreConv::listAllGroupsWithMember(const std::string& participantUuid, list<JSONUnique> &groups)
{
     ...
     *       FROM groups g INNER JOIN members m ON g.groupId=m.groupId WHERE m.memberId=?1;
     */
    SQLITE_CHK(SQLITE_PREPARE(db, selectAllGroupsWithParticipant, -1, &stmt, nullptr));
...
JSONUnique SQLiteStoreConv::listGroup(const string &groupUuid, int32_t *sqlCode)
{
    sqlite3_stmt *stmt;
    int32_t sqlResult;
    JSONUnique jsonUnique;
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectGroup, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
    ...
    if (sqlResult == SQLITE_ROW) {
        jsonUnique = createGroupJson(stmt);
    }
...
int32_t SQLiteStoreConv::modifyGroupMaxMembers(const string &groupUuid, int32_t maxMembers)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, updateGroupMaxMember, -1, &stmt, nullptr));
...
pair<int32_t, time_t> SQLiteStoreConv::getGroupAttribute(const string& groupUuid, int32_t* sqlCode) const
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectGroupAttributeSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
int32_t SQLiteStoreConv::setGroupAttribute(const string& groupUuid, int32_t attributeMask)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, setGroupAttributeSql, -1, &stmt, nullptr));
...
int32_t SQLiteStoreConv::clearGroupAttribute(const string& groupUuid, int32_t attributeMask) {
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, clearGroupAttributeSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_int(stmt, 1, attributeMask));
    SQLITE_CHK(sqlite3_bind_int64(stmt, 2, time(nullptr)));
    SQLITE_CHK(sqlite3_bind_text(stmt, 3, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
int32_t SQLiteStoreConv::setGroupName(const string& groupUuid, const string& name)
{
    ...
    sqlResult = SQLITE_PREPARE(db, setGroupNewName, -1, &stmt, nullptr);
...
int32_t SQLiteStoreConv::setGroupBurnTime(const string& groupUuid, int64_t timeInSeconds, int32_t mode) {
    ...
    sqlResult = SQLITE_PREPARE(db, setGroupBurn, -1, &stmt, nullptr);
...
int32_t SQLiteStoreConv::setGroupAvatarInfo(const string& groupUuid, const string& avatarInfo) {
    ...
    sqlResult = SQLITE_PREPARE(db, setGroupAvatar, -1, &stmt, nullptr);
...
static int32_t incrementMemberCount(sqlite3* db, const string& groupUuid) {
    ...
    sqlResult = SQLITE_PREPARE(db, incrementGroupMemberCount, -1, &stmt, nullptr);
...
static int32_t decrementMemberCount(sqlite3* db, const string& groupUuid) {
    ...
    sqlResult = SQLITE_PREPARE(db, decrementGroupMemberCount, -1, &stmt, nullptr);
    sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC);
    if (sqlResult != SQLITE_OK) {
        goto cleanup;
    }
...
static int32_t setMemberCount(sqlite3* db, const string& groupUuid, int32_t count) {
    ...
    sqlResult = SQLITE_PREPARE(db, setGroupMemberCount, -1, &stmt, nullptr);
...
int32_t SQLiteStoreConv::insertMember(const string &groupUuid, const string &memberUuid)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, insertMemberSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
int32_t SQLiteStoreConv::deleteMember(const string &groupUuid, const string &memberUuid)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, removeMember, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
    SQLITE_CHK(sqlite3_bind_text(stmt, 2, memberUuid.data(), static_cast<int32_t>(memberUuid.size()), SQLITE_STATIC));
...
int32_t SQLiteStoreConv::deleteAllMembers(const string &groupUuid) {
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, removeAllMembers, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
static JSONUnique createMemberJson(sqlite3_stmt *stmt)
{
    auto jsn = make_unique<json>(json());

    jsn->emplace(GROUP_ID,  (const char*)sqlite3_column_text(stmt, 0));
    jsn->emplace(MEMBER_ID, (const char*)sqlite3_column_text(stmt, 1));
    jsn->emplace(MEMBER_ATTRIBUTE, sqlite3_column_int(stmt, 2));
    jsn->emplace(MEMBER_MOD_TIME, sqlite3_column_int64(stmt, 3));
...
int32_t SQLiteStoreConv::getAllGroupMembers(const string &groupUuid, list<JSONUnique> &members)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectGroupMembers, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
    ...
    while (sqlResult == SQLITE_ROW) {
        ...
        members.push_back(createMemberJson(stmt));
        sqlResult = sqlite3_step(stmt);
    }
...
int32_t SQLiteStoreConv::getAllGroupMemberUuids(const string &groupUuid, list<string> &members)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectGroupMemberUuids, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
JSONUnique
SQLiteStoreConv::getGroupMember(const string &groupUuid, const string &memberUuid, int32_t *sqlCode)
{
    sqlite3_stmt *stmt;
    int32_t sqlResult;
    JSONUnique uniqueJson;
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectMember, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
    SQLITE_CHK(sqlite3_bind_text(stmt, 2, memberUuid.data(), static_cast<int32_t>(memberUuid.size()), SQLITE_STATIC));
    ...
    if (sqlResult == SQLITE_ROW) {
        uniqueJson = createMemberJson(stmt);
    }
...
pair<int32_t, time_t> SQLiteStoreConv::getMemberAttribute(const string &groupUuid, const string &memberUuid, int32_t *sqlCode)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectMemberAttributeSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
    SQLITE_CHK(sqlite3_bind_text(stmt, 2, memberUuid.data(), static_cast<int32_t>(memberUuid.size()), SQLITE_STATIC));
...
int32_t SQLiteStoreConv::setMemberAttribute(const string &groupUuid, const string &memberUuid, int32_t attributeMask)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, setMemberAttributeSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_int(stmt,  1, attributeMask));
    SQLITE_CHK(sqlite3_bind_int64(stmt,2, time(nullptr)));
    SQLITE_CHK(sqlite3_bind_text(stmt, 3, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
int32_t SQLiteStoreConv::clearMemberAttribute(const string &groupUuid, const string &memberUuid, int32_t attributeMask)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, clearMemberAttributeSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_int(stmt, 1, attributeMask));
    SQLITE_CHK(sqlite3_bind_int64(stmt,2, time(nullptr)));
    SQLITE_CHK(sqlite3_bind_text(stmt, 3, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
    SQLITE_CHK(sqlite3_bind_text(stmt, 4, memberUuid.data(), static_cast<int32_t>(memberUuid.size()), SQLITE_STATIC));
...
int32_t SQLiteStoreConv::memberListHash(const string &groupUuid, uint8_t *hash)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectForHash, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
...
bool SQLiteStoreConv::isMemberOfGroup(const string &groupUuid, const string &memberUuid, int32_t *sqlCode) {
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, isMemberOfGroupSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupUuid.data(), static_cast<int32_t>(groupUuid.size()), SQLITE_STATIC));
    SQLITE_CHK(sqlite3_bind_text(stmt, 2, memberUuid.data(), static_cast<int32_t>(memberUuid.size()), SQLITE_STATIC));
...
bool SQLiteStoreConv::isGroupMember(const string &memberUuid, int32_t *sqlCode) {
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, isGroupMemberSql, -1, &stmt, nullptr));
...
int32_t SQLiteStoreConv::insertChangeSet(const string &groupId, const string& changeSet)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, insertChangeSetSql, -1, &stmt, nullptr));
...
int32_t SQLiteStoreConv::removeChangeSet(const string &groupId)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, removeChangeSetSql, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupId.data(), static_cast<int32_t>(groupId.size()), SQLITE_STATIC));
...
int32_t SQLiteStoreConv::getGroupChangeSet(const string &groupId, string* changeSet)
{
    ...
    SQLITE_CHK(SQLITE_PREPARE(db, selectChangeSet, -1, &stmt, nullptr));
    SQLITE_CHK(sqlite3_bind_text(stmt, 1, groupId.data(), static_cast<int32_t>(groupId.size()), SQLITE_STATIC));
    ...
    while (sqlResult == SQLITE_ROW) {
        ...
        auto len = static_cast<size_t>(sqlite3_column_bytes(stmt, 1));
